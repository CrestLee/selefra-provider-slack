...
func (x *TableSlackConnectionGenerator) GetColumns() []*schema.Column {
	return []*schema.Column{
		table_schema_generator.NewColumnBuilder().ColumnName("enterprise_id").ColumnType(schema.ColumnTypeString).Description("ID of the enterprise grid. null if not an enterprise workspace.").Build(),
		table_schema_generator.NewColumnBuilder().ColumnName("bot_id").ColumnType(schema.ColumnTypeString).Description("ID of the bot making the connection. null if not a bot.").Build(),
		...
		table_schema_generator.NewColumnBuilder().ColumnName("url").ColumnType(schema.ColumnTypeString).Description("URL of the workspace.").Build(),
		...
		table_schema_generator.NewColumnBuilder().ColumnName("team_id").ColumnType(schema.ColumnTypeString).Description("ID of the workspace team.").Build(),
		table_schema_generator.NewColumnBuilder().ColumnName("user_id").ColumnType(schema.ColumnTypeString).Description("ID of the user making the connection.").Build(),
	}
}
